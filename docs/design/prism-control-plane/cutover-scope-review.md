# Cutover Scope Review for Prism Control Plane

This note answers one specific question:

When Prism cuts over to the control plane, which capabilities must exist in the canonical shell, and which should explicitly *not* be added?
...
These are not optional if the canonical shell is going to replace the previous production UI.
...
- do not recreate a standalone `Auth Profiles` page in the control plane
...
- the canonical shell should not only succeed on the happy path
...
These capabilities are not necessary as separate product surfaces, even though earlier dashboards or the broader ecosystem may suggest them.

### 1. Do not rebuild the legacy page tree in the control plane
...
- include every operator-critical capability listed in `What The Control Plane Must Include`
