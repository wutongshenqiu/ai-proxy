# Rollout Strategy for a Greenfield Prism Control Plane

This note answers a specific implementation question:

If Prism can ignore legacy UI compatibility, how should it still ship safely?

The answer is:

- rebuild the control-plane UI greenfield
- keep runtime-truth backend primitives where they are already correct
- keep the old dashboard in production until the new control plane is fully ready
...
- keep the old dashboard as the only production UI
...
- keep the old dashboard available only as a temporary rollback or emergency fallback if needed
...
### 4. Use the old dashboard only as a pre-cutover production UI

The old UI can remain live while the new one is still incomplete.
...
- interim production UI: old dashboard until readiness
